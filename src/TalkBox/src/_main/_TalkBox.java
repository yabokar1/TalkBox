...
public class _TalkBox extends Application {
	...
	private AudioSampleList AudioSample(ButtonPanel panel, ProfileList profile) { // When does this method get called?
	...
	public void AddProfileBox(RootView root, ButtonPanel panel, ProfileList profile) throws FileNotFoundException {
		...
		section2.getChildren().addAll(new Label("Profiles"), profile.getTreeItem(), buttons.enterProfileTextField(profile),
				new Label("Audio"), this.AudioSample(panel, profile).getList(), buttons.addRecordArea());
	...
	public VBox addButtonPanelAndLabel(ScrollPane scrollpane) {
		...
		TalkBoxButtons buttons = new TalkBoxButtons();
		section1.getChildren().addAll(buttons.Menu().getMenu(), buttons.headerLabel(), scrollpane, buttons.addLaunchButton());
		return section1;
	}
